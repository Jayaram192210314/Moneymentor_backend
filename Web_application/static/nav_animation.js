document.addEventListener("DOMContentLoaded", () => {
    ...
    // We append the activeElement to the #nav container
    ...
    // Initialize animation positioning
    const initAnimation = () => {
        // Bootstrap scrollspy adds 'active' class to a.nav-link
        let activeLink = navElement.querySelector("ul li a.active");
        
        // If no active link is found initially, pick the first one
        if (!activeLink) {
            activeLink = navElement.querySelector("ul li a.nav-link");
    ...
    // Wait for fonts to be ready before calculating offset
    ...
    const links = navElement.querySelectorAll("ul li a.nav-link");
    
    // Listen for bootstrap's scrollspy events to trigger animation
    window.addEventListener('activate.bs.scrollspy', function (e) {
        const targetLink = e.relatedTarget;
        const button = document.querySelector(`a[href="${targetLink}"]`);
        if (!button) return;

        triggerAnimation(button);
    });

    // Also handle manual clicks
    ...
    function triggerAnimation(button) {
        const active = navElement.querySelector("ul li a.active");
        if (!active || active === button) return;

        const linksArray = Array.from(links);
        const index = linksArray.indexOf(button);
        const oldIndex = linksArray.indexOf(active);
        ...
        // We manually update classes for the animation to look smooth instantly
    ...
    const createSVG = (element) => {
        // Adjusted colors to match the primary theme #8b5cf6
        element.innerHTML = `
        ...
        <div class="strike">
            <svg viewBox="0 0 114 12" preserveAspectRatio="none">
                <g fill="none" stroke="var(--primary, #8b5cf6)" stroke-width="0.75" stroke-linecap="round">
